...
// NESDL: changed int to size_t (compiler warning)
					std::size_t pos = s.find(phrase);
// NESDL_END
...
// NESDL: added lines to load existing values first
						//open file in fstream::in, to read existing values
						this->file.open(filename, std::fstream::in);
						load();
						this->file.close();
						//open again in fstream::out, to be ready for writes
// NESDL_END
						//open the fstream
...
// NESDL: created alt write function for our needs
					//a function for write mode that saves changes from the buffer (the data variable) to the file
					//(a simpler secondary version that only saves data entries, and does not write duplicates)
					bool write_changes_unique() {
						//if the file is in the wrong mode, or no changes have been made
						if (this->mode != WRITE || this->data.empty()) {
							//abort
							return false;
							...
							//read the entries from the map, and output the values from the buffer
							for (std::pair<std::string, std::string> kv : this->data) {
								this->file << kv.first << '=' << kv.second << '\n';
							}
